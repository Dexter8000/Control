...
class VacacionesManager {
    ...
    async crearVacaciones(datosVacaciones) {
        ...
        return new Promise((resolve, reject) => {
            this.db.run(query, [
                empleado_id, fecha_inicio, fecha_fin, fechaRetorno.toISOString().split('T')[0],
                diasTotales, tipo_vacaciones, motivo, aprobado_por, notas, añoPeriodo
            ], function(err) {
                if (err) reject(err);
                else resolve({ id: this.lastID, diasTotales, fechaRetorno: fechaRetorno.toISOString().split('T')[0] });
            });
        });
    }
    ...
    async calcularInfoVacaciones(empleado_id) {
        ...
        if (vacacionesActivas) {
            ...
            info.diasTranscurridos = Math.ceil((hoy - fechaInicio) / (1000 * 60 * 60 * 24)) + 1;
            info.diasRestantes = Math.ceil((fechaRetorno - hoy) / (1000 * 60 * 60 * 24));
            ...
            info.diasHastaProximas = Math.ceil((fechaInicio - hoy) / (1000 * 60 * 60 * 24));
        }
