...
const db = new sqlite3.Database(dbPath, (err) => {
    ...
    `, (err, usuarios) => {
        ...
        usuarios.forEach(user => {
            ...
            console.log(`🆔 ID: ${user.id} | Usuario: ${user.usuario} | ${estado} | Rol: ${user.rol}`);
        });
        ...
        idsProblematicos.forEach(id => {
            db.get('SELECT * FROM usuarios WHERE id = ?', [id], (err, user) => {
                if (err) {
                    ...
                    console.log(`⚠️  ID ${id} ENCONTRADO: ${user.usuario} - Activo: ${user.activo ? 'Sí' : 'No'}`);
        ...
        setTimeout(() => {
            db.run('DELETE FROM usuarios WHERE activo = 0', function(err) {
                if (err) {
                    console.error('❌ Error eliminando usuarios inactivos:', err.message);
                } else {
