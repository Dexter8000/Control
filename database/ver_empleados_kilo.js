...
const db = new sqlite3.Database(dbPath, (err) => {
  ...
  db.all('SELECT * FROM empleados ORDER BY id LIMIT 15', (err, empleados) => {
    ...
    console.log(
      `👥 EMPLEADOS ENCONTRADOS: ${empleados.length} (mostrando primeros 15)\n`
    );
    ...
    db.get('SELECT COUNT(*) as total FROM empleados', (err, result) => {
      ...
      // Contar por estado
      db.all(
        'SELECT activo, COUNT(*) as cantidad FROM empleados GROUP BY activo',
        (err, estados) => {
          if (!err && estados.length > 0) {
            console.log('\n📈 POR ESTADO:');
            estados.forEach((estado) => {
              console.log(
                `   ${estado.activo || 'Sin estado'}: ${estado.cantidad} empleados`
              );
            });
          }
          ...
          db.all(
            'SELECT departamento, COUNT(*) as cantidad FROM empleados WHERE departamento IS NOT NULL GROUP BY departamento ORDER BY cantidad DESC LIMIT 5',
            (err, departamentos) => {
              if (!err && departamentos.length > 0) {
                console.log('\n🏢 TOP 5 DEPARTAMENTOS:');
                departamentos.forEach((dept) => {
                  console.log(
                    `   ${dept.departamento}: ${dept.cantidad} empleados`
                  );
                });
              }
              ...
              // Contar por rango
              db.all(
                'SELECT rango, COUNT(*) as cantidad FROM empleados WHERE rango IS NOT NULL GROUP BY rango ORDER BY cantidad DESC LIMIT 5',
                (err, rangos) => {
                  if (!err && rangos.length > 0) {
                    console.log('\n🎖️ TOP 5 RANGOS:');
                    rangos.forEach((rango) => {
                      console.log(
                        `   ${rango.rango}: ${rango.cantidad} empleados`
                      );
                    });
                  }
