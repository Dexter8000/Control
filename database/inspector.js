...
async function inspectDatabase() {
  ...
  try {
    ...
    for (const table of tables) {
      ...
      columns.forEach((col) => {
        console.log(
          `   ${col.name} (${col.type}) ${col.pk ? '[PK]' : ''} ${col.notnull ? '[NOT NULL]' : ''}`
        );
      });
      ...
      if (table.name === 'usuarios') {
        ...
        const users = await new Promise((resolve, reject) => {
          db.db.all(
            'SELECT id, usuario, rol, nombre, apellido, activo FROM usuarios',
            (err, rows) => {
              if (err) reject(err);
              else resolve(rows);
            }
        ...
        users.forEach((user) => {
          console.log(
            `   • ID: ${user.id} | Usuario: ${user.usuario} | Rol: ${user.rol} | Activo: ${user.activo ? 'Sí' : 'No'}`
          );
        });
        console.log('');
      }
      ...
      if (table.name === 'empleados') {
        ...
        const empleados = await new Promise((resolve, reject) => {
          db.db.all(
            'SELECT id, placa, rango, nombre, apellido, departamento_id FROM empleados LIMIT 5',
            (err, rows) => {
              if (err) reject(err);
              else resolve(rows);
            }
        ...
        empleados.forEach((emp) => {
          console.log(
            `   • ${emp.id} | Placa: ${emp.placa} | ${emp.rango} ${emp.nombre} ${emp.apellido} | Depto: ${emp.departamento_id}`
          );
        });
