...
const db = new sqlite3.Database(dbPath, (err) => {
    ...
    db.get("SELECT * FROM empleados WHERE id = 267291", (err, empleado) => {
        ...
        db.get("SELECT * FROM empleados WHERE placa = '99999'", (err, empleado) => {
            ...
            db.get("SELECT * FROM empleados WHERE nombre LIKE '%LEE%' AND nombre LIKE '%Morales%'", (err, empleado) => {
                if (err) {
                ...
                console.log('===============================');
                db.all("SELECT id, placa, nombre, rango FROM empleados ORDER BY id DESC LIMIT 10", (err, empleados) => {
                    if (err) {
                        console.error('❌ Error:', err.message);
                    } else {
                        empleados.forEach(emp => {
                            console.log(`ID: ${emp.id} | Placa: ${emp.placa} | ${emp.nombre} | ${emp.rango}`);
                        });
                    }
                    ...
                    console.log('\n📊 RANGO DE IDs:');
                    console.log('================');
                    db.get("SELECT MIN(id) as min_id, MAX(id) as max_id, COUNT(*) as total FROM empleados", (err, stats) => {
                        if (!err) {
