...
console.log('🔍 VERIFICANDO BASE DE DATOS: kilo.db');
console.log('=================================================\n');
...
function verificarContenidoTablas(tables) {
  ...
  tables.forEach((table) => {
    ...
    db.all(`PRAGMA table_info(${tableName})`, (err, columns) => {
      if (err) {
        console.error(
          `❌ Error obteniendo estructura de ${tableName}:`,
          err.message
        );
        return;
      }
      ...
      db.get(`SELECT COUNT(*) as count FROM ${tableName}`, (err, row) => {
        if (err) {
          console.error(
            `❌ Error contando registros en ${tableName}:`,
            err.message
          );
          return;
        }
        ...
        if (tableName === 'usuarios' && row.count > 0) {
          db.all(
            'SELECT id, usuario, rol, activo, fecha_creacion FROM usuarios LIMIT 5',
            (err, users) => {
              if (!err) {
                console.log('   👥 Usuarios (muestra):');
                users.forEach((user) => {
                  console.log(
                    `      • ID: ${user.id} | ${user.usuario} | ${user.rol} | Activo: ${user.activo ? 'Sí' : 'No'}`
                  );
                });
              }
            }
        ...
        if (tableName === 'departamentos' && row.count > 0) {
          db.all(
            'SELECT id, nombre FROM departamentos LIMIT 5',
            (err, depts) => {
              if (!err) {
        ...
        if (tableName === 'empleados' && row.count > 0) {
          db.all(
            'SELECT id, nombre, apellido, placa, rango FROM empleados LIMIT 3',
            (err, emps) => {
              if (!err) {
                console.log('   👨‍💼 Empleados (muestra):');
                emps.forEach((emp) => {
                  console.log(
                    `      • ${emp.id} | ${emp.nombre} ${emp.apellido} | Placa: ${emp.placa}`
                  );
                });
              }
            }
  ...
  db.all("SELECT name FROM sqlite_master WHERE type='table'", (err, tables) => {
    ...
    db.all('SELECT usuario, rol, activo FROM usuarios', (err, users) => {
      ...
      if (users.length === 0) {
        ...
        users.forEach((user) => {
          console.log(
            `      • ${user.usuario} (${user.rol}) - ${user.activo ? 'Activo' : 'Inactivo'}`
          );
        });
      }
