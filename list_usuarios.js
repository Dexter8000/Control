...
db.all('PRAGMA table_info(usuarios);', [], (err, columns) => {
  if (err) {
    console.error(
      'Error al obtener la estructura de la tabla usuarios:',
      err.message
    );
    db.close();
    return;
  ...
  db.all(
    'SELECT id, nombre, usuario, email, rol, activo FROM usuarios ORDER BY id;',
    [],
    (err, usuarios) => {
      if (err) {
      ...
      usuarios.forEach((usuario) => {
        console.log(
          `${usuario.id} | ${usuario.nombre} | ${usuario.usuario} | ${usuario.email} | ${usuario.rol} | ${usuario.activo ? 'Sí' : 'No'}`
        );
      });
      ...
      console.log(
        'Si necesitas restablecer una contraseña, usa el sistema de recuperación.'
      );
