    ...
    async (err, usuarios) => {
      ...
      for (const usuario of usuarios) {
        try {
          ...
          console.log(
            `🔒 Hash actual: ${usuario.contrasena.substring(0, 20)}...`
          );
          ...
          if (!esHashValido) {
            ...
            const respuesta = await new Promise((resolve) => {
              readline.question(
                '¿Deseas actualizar el hash de este usuario? (s/n): ',
                resolve
              );
            });
            ...
            if (respuesta.toLowerCase() === 's') {
              const nuevaContrasena = await new Promise((resolve) => {
                readline.question(
                  'Ingresa la nueva contraseña (o presiona Enter para omitir): ',
                  resolve
                );
              });
              ...
              if (nuevaContrasena) {
                try {
                  ...
                  const actualizado = await actualizarHash(
                    usuario.id,
                    nuevoHash
                  );
          ...
          console.error(
            `❌ Error procesando usuario ${usuario.usuario}:`,
            error.message
          );
        }
      }
