...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.querySelectorAll('.pc-card').forEach((card) => {
    card.addEventListener('click', () => {
      ...
      if (empleadosActions) {
        empleadosActions.style.display =
          card.dataset.table === 'empleados' ? 'flex' : 'none';
      }
      if (usuariosActions) {
        usuariosActions.style.display =
          card.dataset.table === 'usuarios' ? 'flex' : 'none';
      }
      if (departamentosActions) {
        departamentosActions.style.display =
          card.dataset.table === 'departamentos' ? 'flex' : 'none';
      }
    });
  });
