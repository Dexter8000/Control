...
async function loadDashboardData() {
  try {
    await Promise.all([loadUsers(), loadInventoryData(), loadEmployeesData()]);
...
function renderUsers() {
  ...
  console.log(
    `Renderizados ${filteredUsers.length} usuarios de ${allUsers.length} total`
  );
}
...
function filterUsers() {
  ...
  const roleFilter =
    document.getElementById('users-role-filter')?.value || 'todos';
...
async function saveUser(event) {
  ...
  try {
    ...
    if (
      ...
      showNotification(
        'El nombre de usuario ya está en uso. Por favor, elija otro nombre.',
        'error'
      );
    } else {
      showNotification(
...
async function confirmDeleteUser() {
  ...
  try {
    ...
    showNotification(
      'Error de conexión al eliminar usuario: ' + error.message,
      'error'
    );
  }
}
...
async function loadInventoryData() {
  try {
    ...
    if (response.ok) {
      ...
      const perifericoData = principalData.flatMap(
        (item) => item.perifericos || []
      );
...
async function loadEmployeesData() {
  try {
    ...
    if (empleadosResponse.ok) {
      ...
      console.log(
        `✅ Estadísticas de empleados actualizadas: ${empleados.length} total`
      );
    }
  } catch (error) {
...
function filterEmpleados() {
  ...
  filteredEmpleadosData = allEmpleadosData.filter((empleado) => {
    ...
    const matchesDepartamento =
      !departamentoFilter ||
      empleado.departamento_nombre === departamentoFilter;
...
function updateInventoryCounters(principal, periferico) {
  ...
  if (principalCount)
    principalCount.textContent = principal ? principal.length : 0;
  if (perifericoCount)
    perifericoCount.textContent = periferico ? periferico.length : 0;
}
...
function updateEmployeesTable(empleados) {
  ...
  console.log(
    `✅ Tabla de empleados actualizada con ${empleados.length} registros`
  );
}
...
window.mostrarModalVacaciones = async function mostrarModalVacaciones(
  empleadoId
) {
...
async function eliminarVacaciones(empleadoId) {
  if (
    !confirm(
      '¿Está seguro de que desea eliminar las vacaciones de este empleado?'
    )
  ) {
    return;
  }
...
function calcularCumpleanos(fechaNacimiento) {
  ...
  const diasHasta = Math.ceil(
    (cumpleañosEsteAño - hoy) / (1000 * 60 * 60 * 24)
  );
