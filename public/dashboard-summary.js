...
async function cargarDatosDashboard() {
  try {
    ...
    const resumenRangosDepto = dataRangosDepto.detalle
      ...
      .join(', ');
    document.getElementById('resumenRangosPorDepartamentos').textContent =
    ...
    const resumenCantidadRangos = dataCantidadRangos.detalle
      ...
      .join(', ');
    document.getElementById('resumenCantidadRangos').textContent =
    ...
    document.getElementById('totalDepartamentos').textContent =
    ...
    const datosIncompletosCard = document.getElementById(
      'datosIncompletosCard'
    );
    ...
    if (dataIncompletos.count > 0) {
      ...
      listaIncompletos.innerHTML = dataIncompletos.ids
        .slice(0, 5)
        .map((id) => `<li>ID: ${id}</li>`)
        .join('');
