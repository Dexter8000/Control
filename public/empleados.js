...
async function initializeDepartmentSelectors() {
  ...
  const modalDepartmentSelect = document.querySelector(
    '#empleado-modal select[name="departamento"]'
  );
  if (modalDepartmentSelect) {
    modalDepartmentSelect.innerHTML =
...
async function updateDepartmentFilter() {
  ...
  let opciones = ['<option value="">Todos los departamentos</option>']
    .concat(
      departamentos.map(
        (d) => `<option value="${d.nombre}">${d.nombre}</option>`
      )
    )
    .join('');
...
function setupEventListeners() {
  // Filtros
  document
    .getElementById('busqueda-global')
    .addEventListener('input', debounce(applyFilters, 300));
  document
    .getElementById('filtro-departamento')
    .addEventListener('change', applyFilters);
  document
    .getElementById('filtro-rango')
    .addEventListener('change', applyFilters);
  document
...
async function loadEmpleados() {
  try {
    ...
    showError(
      'No se pudieron cargar los empleados. Intente recargar la página.'
    );
...
function updateKPIs(datosRangos = []) {
  ...
  const rangoMasFrecuente = Object.entries(rangoCount).sort(
    ([, a], [, b]) => b - a
  )[0];
  ...
  const deptMasFrecuente = Object.entries(deptCount).sort(
    ([, a], [, b]) => b - a
  )[0];
  ...
  document.getElementById('distribucion-rangos').innerHTML =
    topDepartamentos ||
  ...
  filteredEmpleados = allEmpleados.filter((empleado) => {
    ...
    const matchesBusqueda =
      ...
      (empleado.correo_electronico &&
        empleado.correo_electronico.toLowerCase().includes(busqueda)) ||
    ...
    const matchesEstado =
      ...
      (estado === 'Activo' &&
        (empleado.activo === 'Sí' || empleado.estado === 'Activo'));

    return (
      matchesBusqueda && matchesDepartamento && matchesRango && matchesEstado
    );
  });
...
async function showNuevoEmpleadoModal() {
  ...
  document.getElementById('modal-title').innerHTML =
    '<i data-feather="user-plus"></i> Agregar Nuevo Empleado';
...
async function editEmpleado(empleadoId) {
  try {
    ...
    if (data.success && data.empleado) {
      ...
      document.getElementById('modal-title').innerHTML =
        '<i data-feather="edit-2"></i> Editar Empleado';
      ...
      document.querySelector('[name="fecha_nacimiento"]').value =
        empleado.fecha_nacimiento
          ...
          : '';
      document.querySelector('[name="correo_electronico"]').value =
        empleado.correo_electronico || empleado.correo || '';
    ...
    showNotification(
      '❌ Error cargando datos del empleado: ' + error.message,
      'error'
    );
  }
}
...
async function confirmDeleteEmpleado() {
  if (!empleadoToDelete) {
    showNotification(
      'Error: No hay empleado seleccionado para eliminar',
      'error'
    );
    return;
  }
...
async function saveEmpleado() {
  ...
  try {
    ...
    console.log(
      `${isEditing ? '📝' : '➕'} ${isEditing ? 'Actualizando' : 'Creando'} empleado:`,
      empleadoData
    );
...
async function uploadCSV() {
  ...
  reader.onload = async function (e) {
    ...
    if (sucessCount > 0) {
      await loadEmpleados();
      showNotification(
        `${sucessCount} empleados cargados exitosamente`,
        'success'
      );
    }
  };
