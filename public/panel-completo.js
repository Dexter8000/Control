...
async function init() {
  ...
  document
    .getElementById('departamentoFilter')
  ...
  document
    .querySelectorAll('#tabla-rangos-departamento th')
    .forEach((th) =>
      th.addEventListener('click', () => ordenarRD(th.dataset.col))
    );
  document
    .querySelectorAll('#tabla-cantidad-rangos th')
    .forEach((th) =>
...
function filtrar() {
  ...
  filtrados = empleados.filter((e) => {
    const matchTexto = `${e.nombre || ''} ${e.apellido || ''}`
      .toLowerCase()
      .includes(texto);
    const matchDepto =
      !depto || (e.departamento_nombre || e.departamento) === depto;
...
function filtrarRD() {
  const t = document
    .getElementById('buscar-rangos-departamento')
    .value.toLowerCase();
  filtradosRD = rangosDepartamento.filter(
    (d) =>
...
function filtrarCR() {
  const t = document
    .getElementById('buscar-cantidad-rangos')
    .value.toLowerCase();
